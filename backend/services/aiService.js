...
const EXERCISE_POOLS = require('../config/exercisePools.json');
...
4. Format responses cleanly in markdown. Do NOT wrap conversational advice in JSON blocks.`;
    ...
    logger.error({ err: error.response?.data || error.message }, 'Chat AI Error');
...
Output strictly raw JSON without explanations or markdown tags.`;
  ...
  try {
    ...
    logger.error({ err: error.response?.data || error.message }, 'Modify Workout AI Error');
    return null;
  }
};
